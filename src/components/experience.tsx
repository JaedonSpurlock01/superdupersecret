        ...
        {work.map((item, index) => (
          <div key={`${item.company}-${item.date}`} className="flex gap-4">
            <div className="relative flex w-6 justify-center translate-y-5">
              <div
                className={cn(
                  "z-10 mt-1 h-2.5 w-2.5 rounded-full bg-muted-foreground ring-3 ring-primary/15",
                  index === 0 && "animate-pulse bg-green-500 ring-green-500/15",
                )}
              />
            </div>
              ...
              <div className="flex flex-wrap justify-between items-start gap-x-2 gap-y-1">
                <div>
                  ...
                  <div className="flex flex-col text-xs text-muted-foreground">
                    ...
                    <Badge
                      variant="secondary"
                      className="mt-2 text-[10px] rounded-sm text-neutral-700 dark:text-neutral-300 bg-neutral-200 dark:bg-neutral-800 border border-border/40"
                    >
                      {item.date}
                    </Badge>
                  </div>
