...
export function AnimatedTextLink({ href, children, className, arrowSide = "right", noArrow = false }: AnimatedTextLinkProps) {
    return (
        <Link
            href={href}
            className={cn(
        ...
        >
            {
                arrowSide === "left" && !noArrow && (<HugeiconsIcon
                    icon={ArrowLeft}
            ...
            {
                arrowSide === "right" && !noArrow && (<HugeiconsIcon
                    icon={ArrowRight}
                    className="size-3.5 group-hover:translate-x-0.5 group-hover:text-black dark:group-hover:text-neutral-200 transition-all duration-400"
                />)
            }
            <span className={
                cn("absolute -bottom-1 w-0 h-px bg-black dark:bg-neutral-200 group-hover:w-[calc(100%-1.2rem)] transition-all duration-400", !noArrow && arrowSide === "right" ? "left-0" : "right-0")
            } />
        </Link>
    );
}
